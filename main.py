...
async def get_developers():
    try:
        db = await connection()
        developers =await db.developers.find().to_list(100)
        for i in developers:i['_id'] = str(i['_id'])
...
async def delete_developers(id:str):
    try:
        db = await connection()
        developer = await db.developers.find_one({'_id':ObjectId(id)})
        if not developer:
        ...
        return JSONResponse(status_code=201, content={"message":"Desarrollador eliminado con exito :)"})
    except Exception as e:
        print(e)
        return JSONResponse(status_code=500, content={"message":"ha ocurrido un error"})
